...
const SuggestRecipes = () => {
    const searchByName = async (name: string, searchAll:boolean) => {
        if (searchAll){
    ...
    const searchByIngredients = async (ingredients: string[], searchAll:boolean) => {
        if (searchAll){
    ...
    return { searchByName, searchByIngredients }
}
